  ...
  <div class="app-container">
    <el-form :inline="true" ref="taskFrm" :model="mainFrm">
      <el-form-item label="流水号">
        <el-input placeholder="请输入流水号" v-model="mainFrm.number" clearable></el-input>
      </el-form-item>
...
export default {
  ...
  computed: {
    listType() {
      switch (this.$route.meta.title) {
        case 'matchSloganList': // 口号
          return 33;
        case 'matchEmblemList': // 会徽
          return 9;
        case 'matchMascotList': // 吉祥物
          return 34;
        case 'matchBlessingList': // 祝福
          return 81;
      }
    },
    start_time() {
  ...
  methods: {
    getTableOptions() {
      // 每种征集的表格展示内容不同,过滤一些数据
      this.tableOptions.map((item, index) => {
        if (
          this.$route.meta.title === 'matchSloganList' &&
          item.label === '作品图片'
        ) {
          this.tableOptions.splice(index, 1);
        } else if (
          this.$route.meta.title === 'matchBlessingList' &&
          (item.label === '作品图片' || item.label === '流水号')
        ) {
          this.tableOptions.splice(index, 1);
          this.getTableOptions();
        }
      ...
      Api.getSolication({
        ...
        .then(data => {
          ...
          data.result.map(item => {
            if (this.tableOptions[0].prop === 'img' && item.img) {
              item.img = `<a target="_blank" href="${item.img}"><img src="${item.img}" style="display:block;width: 100%;" /></a>`;
            } else {
              item.img = '用户未上传图片';
            }
    ...
    actionHandler(type, info) {
      if (type === 'examine') {
        this.$router.push(
          `/activityCenter/matchSolicitation/details?id=${info.id}`
        );
      }
    },
    exportData(type) {
      ...
      let url = `${process.env.BASE_API}/content/util/official-collects/export?`;
