<template>
  <div class="app-container">
    <el-form :inline="true" ref="taskFrm" :model="mainFrm">
      <el-form-item prop="lsh" label="流水号">
        <el-input placeholder="请输入流水号" v-model="mainFrm.lsh" clearable></el-input>
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-tooltip class="item" effect="dark" content="导出符合搜索条件的所有数据" placement="top">
          <el-button type="primary" @click="exportData('all')">全部导出</el-button>
        </el-tooltip>
...
export default {
  ...
  methods: {
      ...
      Api.getCoopers({
        ...
        .then(data => {
          data.result.forEach(item => {
            item.post_time = parseTime(item.post_time);
          });
    ...
    deleteHandler(info) {
      // 删除
      this.$confirm(`确认删除 <b>${info.name}</b> 的合作申请吗？`, '删除', {
    ...
    actionHandler(type, info) {
      ...
      if (type === 'examine') {
        this.$router.push(
          `/activityCenter/matchCooperation/details?id=${info.id}`
        );
      }
    },
    exportData(type) {
      ...
      let url = `${process.env.BASE_API}/content/util/cooperations/export?`;
