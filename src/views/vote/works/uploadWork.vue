 ...
 * @LastEditTime: 2019-08-30 09:33:31
  ...
  <div class="app-container">
    <el-form
      ...
      <el-form-item prop="detail" label="添加作品">
        ...
        <tinymce v-else-if="topicFrm.type == 4" placeholder="请输入内容" :height="300" v-model="topicFrm.detail"/>
        <el-upload
          v-else
          ...
          <video v-if="topicFrm.detail&&topicFrm.type == 1" :src="topicFrm.detail" controls="controls" width="600px" height="380px" style="margin-left:20px"></video>
          <img v-else-if="topicFrm.detail&&topicFrm.type == 2" :src="topicFrm.detail" class="avatar">
          <audio v-else-if="topicFrm.detail&&topicFrm.type == 5" :src="topicFrm.detail" controls="controls" style="margin-left:20px"></audio>
  ...
  export default {
    ...
    methods: {
      ...
      getStage() {
        ...
        this.stage = [];
        this.task = [];
        Activity.stage.list(this.topicFrm.activity_id, {
      ...
      getTask() {
        this.topicFrm.mission_id = '';
        this.task = [];
        Activity.task.list( this.topicFrm.stage_id, {
      ...
      beforeWorkUpload(file) {
        ...
        if (this.topicFrm.type - 0 === 1 && file.type.split('/')[0] !== 'video') {
          ...
          return false;
        } else if (this.topicFrm.type - 0 === 2 && file.type.split('/')[0] !== 'image') {
          this.$message({
            type: 'error',
            message: '请上传图片类作品'
          });
          return false;
        } else if (this.topicFrm.type - 0 === 5 && file.type.split('/')[0] !== 'audio') {
          this.$message({
            type: 'error',
            message: '请上传音频类作品'
          });
