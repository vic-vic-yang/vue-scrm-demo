  ...
  <div class="app-container task-list">
    <el-form
      ...
      <div v-if="active === 0">
        ...
        <el-form-item prop="finish_amount" label="任务达标奖励积分额度">
          <el-input
            v-model="taskFm.finish_amount"
        ...
        <el-form-item prop="start_time" label="开始时间">
          ...
          <el-form-item prop="end_time" label="结束时间">
            <el-date-picker
              :editable="false"
              :default-value="taskFm.start_time"
              v-model="taskFm.end_time"
              type="datetime"
    ...
    <el-dialog
      ...
      <el-row style="padding: 10px 0 20px;">
        <template v-for="item in edit_hint">
          <el-col :span="12"> {{item.name}}： { {{item.field}} }</el-col>
        </template>
      </el-row>
      ...
      <el-form
        ...
        <el-form-item label="文本信息">
          <div style="width: 680px;">
            <tinymce placeholder="文本信息" :height="350" :toolbar="toolbar" :menubar="menubar" :keywords="edit_hint" :keywordsShow="true" :miniLinkShow="true" :uploadImgShow="false" v-model="module_cont.content"/>
          </div>
        </el-form-item>
...
<script>
  ...
  import missionTask from '@/api/mission/index';
  ...
  export default {
    ...
    methods: {
      getDetail() {
        ...
        missionTask.awardDetails({ type: 201 }).then(res => {
          res.pic = res.pic.length !== 0 ? res.pic : [{ id: 0, file_path: '' }];
          res.start_time = (res.start_time === '' || res.start_time * 1 === 0) ? '' : parseTime(res.start_time);
          res.end_time = (res.end_time === '' || res.end_time * 1 === 0) ? '' : parseTime(res.end_time);
      ...
      save() {
        this.$refs.taskFm.validate(valid => {
          if (valid) {
            ...
            (this.action === 'modify' ? missionTask.awardEdit(taskFm) : missionTask.awardAdd(taskFm))
              .then(() => {
                this.$router.back();
              });
          }
